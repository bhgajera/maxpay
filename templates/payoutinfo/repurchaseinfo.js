...
.controller('PurchasepayoutCtrl', function($scope, $rootScope,$cordovaToast,PayoutinfoService, $ionicLoading,$ionicHistory, $ionicNavBarDelegate,$state, $stateParams) {      
  $scope.showLoading = function() {
    ...
    $scope.page_no = 1;
    $scope.moreData = true;
    $scope.purchasereports = [];
    $scope.showpurchasereports = function(){
    $scope.showLoading();      
      PayoutinfoService.dailypayoutReport().success(function(res){
        $scope.hideLoading();        
          //$scope.$broadcast('scroll.infiniteScrollComplete');
        console.log("purchasereports >> "+JSON.stringify(res));
        //alert(JSON.stringify(res));
        $scope.purchasereports = $scope.purchasereports.concat(res.output);
        /*$scope.$broadcast('scroll.infiniteScrollComplete');
        $scope.page_no++;
